...
for theta=(0:ht:90)
    for b=(-2:hb:2)
        ...
        g(:,:,j)=max(0,csb).^2;
        dg1(:,:,j)=(max(0,csb+cos(theta)*(delta)).^2-max(0,csb-cos(theta)*(delta)).^2)/(2*delta);
        dg2(:,:,j)=(max(0,csb+sin(theta)*(delta)).^2-max(0,csb-sin(theta)*(delta)).^2)/(2*delta);
        j=j+1;
    end
end
...
fqpt=f(qpt',qpt);
uk=zeros(L,L);%数值解
du1=zeros(L,L);%uk对x1求导的导函数
du2=zeros(L,L);%uk对x2求导的导函数
id = zeros(iter,1);%标记字典中组成uk的基的索引
argmax=zeros(nd,1);%存储|<g,u-uk>|的值
A = zeros(iter,iter); rhs = zeros(iter,1);%求解投影的线性方程组系数矩阵与常数项矩阵
for i=1:iter
    for j=1:nd
        argmax(j)=int2d(g(:,:,j).*(uk-fqpt)+dg1(:,:,j).*du1+dg2(:,:,j).*du2)*h^2;
    end
    [~,id(i)] = max(abs(argmax));
    for j = 1:i
        A(j,i) = int2d(g(:,:,id(j)).*g(:,:,id(i))+dg1(:,:,id(j)).*dg1(:,:,id(i))+dg2(:,:,id(j)).*dg2(:,:,id(i)))*h^2;
        A(i,j) = A(j,i);
    end
    rhs(i) = int2d(g(:,:,id(i)).*fqpt)*h^2;
    ...
    for j = 1:i
        uk=uk+g(:,:,id(j))*C(j);
        du1=du1+dg1(:,:,id(j))*C(j);
        du2=du2+dg2(:,:,id(j))*C(j);
    end
end

u=@(x1,x2) cos(2*pi*x1)*cos(2*pi*x2);
uq=u(qpt',qpt);
mesh(qpt',qpt,uk);
hold on;
mesh(qpt',qpt,uq);
